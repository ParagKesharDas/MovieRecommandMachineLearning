...
res=st.sidebar.radio("Select Any Graph",options=("Content Based Movie Prediction","Collaborative Movie Prediction"))
st.title("WELCOME TO MOVIE RECOMMADATION SYSTEM")
...
if(res=="Content Based Movie Prediction"):
    ...
    userlist=pickle.load(open('ratingsDict.pkl','rb'))
    users=pd.DataFrame(userlist)
    kk=st.selectbox("Choose your User Id: ",users["user"].values)
    ...
    if st.button("Predict"):
        ...
        st.header("Showing recommendations for user:")
        # st.header(f"",pUser)
        st.text("{}".format(pUser))
        print("====" * 9)
        # st.write("====" *9 )
        print("Movies with high ratings from user")
        st.header("Movies with high ratings from user")
        print("----" * 8)
        st.write("----" * 8)
